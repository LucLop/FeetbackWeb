  ...
  export default function TabWidget() {
    ...
    if (error || data.status == false) return <p>Erreur :(</p>;
    ...
    function createDicDatatables(data) {
      ...
      if(data.length){
        data.map(pacient=> {
          ...
          temp["options"] = <><Button onClick={() => PatientProfile(pacient)}>Profil du patient</Button> <DeletePatientsModal patientId={pacient.id}></DeletePatientsModal></>
